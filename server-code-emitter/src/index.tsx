...
export async function $onEmit(context: EmitContext<ServerEmitterOptions>) {
  ...
  // Prepend auto-generated header to all emitted .cs files
  prependHeaderToFiles(context.emitterOutputDir);
}
...
function prependHeaderToFiles(dir: string): void {
  for (const entry of fs.readdirSync(dir, { withFileTypes: true })) {
    const fullPath = path.join(dir, entry.name);
    if (entry.isDirectory()) {
      prependHeaderToFiles(fullPath);
    } else if (entry.name.endsWith(".cs")) {
      const content = fs.readFileSync(fullPath, "utf-8");
      fs.writeFileSync(fullPath, autoGeneratedHeader + content);
    }
  }
}
